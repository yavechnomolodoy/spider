...
VerletJS.prototype.spiderweb = function(origin, radius, segments, depth) {
    ...
    composite.pinnedParticles = [];
    for (i=0;i<segments;i+=2) {
        var particle = composite.particles[i];
        var pinConstraint = new PinConstraint(particle, new Vec2(particle.pos.x, particle.pos.y));
        composite.constraints.push(pinConstraint);
        composite.pinnedParticles.push(particle);
    }
...
VerletJS.prototype.updatePinnedPositions = function() {
    var spiderweb = this.composites[0];
    if (spiderweb && spiderweb.pinnedParticles) {
        for (var i = 0; i < spiderweb.pinnedParticles.length; i++) {
            var particle = spiderweb.pinnedParticles[i];
            for (var j = 0; j < spiderweb.constraints.length; j++) {
                var constraint = spiderweb.constraints[j];
                if (constraint instanceof PinConstraint && constraint.a === particle) {
                    constraint.pos.mutableSet(particle.pos);
                    break;
                }
            }
        }
